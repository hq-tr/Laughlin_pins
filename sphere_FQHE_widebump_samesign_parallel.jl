...
function main()
    ...
    println()
    println("In this version, there are two pins at the north and south poles")
    println("They are identical in magnitude and size (shape)")
    println("Each pin, if the 'special' tag is used, also consists of two parts:")
    println("A negative part including the first k orbitals,")
    println("and a positive part including the (k+1)-th orbital.")
    println("This is flipped (for both pins) if the 'positive' flag is specified.")
    println("============================================================")

    if Lz_cutoff != nothing
        Lzs = collect(-Lz_cutoff:1:Lz_cutoff)
    elseif L_z == nothing && n_el !=nothing
    ...
    for Lz in Lzs
        ...
        if special
            if positive_pin
                @time pin_matrix = 2*sphere_wide_bump_matrix(basis, pin_size) - sphere_wide_bump_matrix(basis,pin_size+1)  + 2*sphere_wide_bump_matrix(basis, pin_size,1.,0.,:south) - sphere_wide_bump_matrix(basis,pin_size+1,1.,0.,:south)
            ...
            if positive_pin
                @time pin_matrix = sphere_wide_bump_matrix(basis, pin_size) + sphere_wide_bump_matrix(basis, pin_size,1.,0.,:south)
                textappend = "_positive"
            else
                @time pin_matrix = -1*(sphere_wide_bump_matrix(basis, pin_size) + sphere_wide_bump_matrix(basis, pin_size,1.,0.,:south))
                textappend = ""
            end
        end

        if !isdir("wide_pin_k_$(pin_size)$(textappend)_samesign") mkdir("wide_pin_k_$(pin_size)$(textappend)_samesign") end
        ...
        Threads.@threads for λ in λ_range
            try
                ...
                dirname = "wide_pin_k_$(pin_size)$(textappend)_samesign/$(outname)_$(intname)_$(pinappendname)_out"
